      ...
      >
        ...
        {uploading ? (
          ...
          <>
            ...
            <p className="mt-1 text-[10px] text-[#52525B]">支持批量选择多个文件，自动分块并嵌入到向量索引</p>
          </>
        )}
      </div>
          ...
          {uploadTasks.map((task, idx) => (
            <div key={idx} className="flex items-center gap-2 px-2 py-1.5 text-xs">
              ...
              <span className={`truncate flex-1 ${
                task.status === "done" ? "text-[#52525B]" :
                task.status === "error" ? "text-[#EF4444]" :
                "text-[#E4E4E7]"
              }`}>
                {task.file.name}
              </span>
            ...
            {collections.map((c) => (
              <div key={c.id} className="flex items-center gap-3 px-3 py-2.5 group">
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate text-sm text-[#E4E4E7]">{c.filename}</p>
                  <p className="text-[10px] text-[#52525B]">
                    {c.chunk_count} 片段 · {new Date(c.created_at).toLocaleDateString("zh-CN")}
                  </p>
                </div>
                <button
