import { getLLM, type LLMConfigOverride } from "@/lib/ai/llm";
...
export async function analyzeStyle(
  ...
  // getLLM() 内部会校验 API Key 是否配置；未配置时抛出统一错误文案
  const { client, model } = getLLM({ timeout: 120_000, override: llmOverride });
